...
tape('Custom levels with conflicting names throw.', function (t) {
  ...
  function makeLogger(levels) {
    new Logger({ token: x, levels: levels, writer });
  }
...
tape('Logger allows specification of minLevel at construction', function (t) {
  ...
  const logger1 = new Logger({ token: x, minLevel: name, writer });

  t.equal(logger1.minLevel, 3, 'by name.');

  const logger2 = new Logger({ token: x, minLevel: 3, writer });

  t.equal(logger2.minLevel, 3, 'by index (num)');

  const logger3 = new Logger({ token: x, minLevel: '3', writer });
...
tape('Logger allows specification of withHostname at construction', function (t) {
  ...
  const logger1 = new Logger({ token: x, withHostname: true, writer });

  t.equal(logger1.withHostname, true, 'withHostname');

  const logger2 = new Logger({ token: x, writer });
...
tape('Error objects are serialized nicely.', function (t) {
  ...
  const logger1 = new Logger({ token: x, writer });
  ...
  const logger2 = new Logger({ token: x, withStack: true, writer });
...
tape('Arguments and regex patterns are serialized.', function (t) {
  ...
  const logger = new Logger({ token: x, writer });
...
tape('Custom value transformer is respected.', function (t) {
  ...
  const logger = new Logger({ token: x, replacer: alwaysKittens, writer });
...
//   const logger = new Logger({ token: tkn, secure: false, writer });
...
//   const logger = new Logger({ token: tkn, secure: true, writer });
...
//   const lvl = defaults.levels[3];
//   const tkn = x;
//   const logger = new Logger({ token: tkn, writer });
//   let count = 0;
...
//   const logger = new Logger({ token: tkn, timestamp: true, writer });
...
//   const logger = new Logger({ token: tkn, withHostname: true, writer });
...
//   const lvl = defaults.levels[3];
//   const tkn = x;
//   const logger = new Logger({ token: x , inactivityTimeout: 300, writer});
